    ...
    function clone(obj) {
            ...
            constructor(mass, anchor, x, y, color, radius) {
                ...
                this.fy = 0.0;
                  ...
                  if (radius) {
                      ...
                      this.radius = radiusPerMass * (this.mass ** 0.333333333);
            }
        ...
        class Simulation {
            constructor() {
                ...
                this.springList = [];
                  this.electricList = [];
                  this.collisionList = [];
                this.interactions = [this.springList, this.electricList, this.collisionList];
                  this.energy0 = 0.0;
           ...
           AddElectric(electric) {
            ...
            Update(dt) {
                ...
                for (b of this.ballList) {
                    if (b.anchor != ANCHOR) {       // skip anchors, because they don't move
        ...
        function MakeString(sim, x, y, shadow) {
            ...
            let anchor1 = sim.AddBall(new Ball(BallMass, ANCHOR, x, y, col1, BallRadiusMeters));
            //let anchor1 = sim.AddBall(new Ball(BallMass, ANCHOR, x, y, col1, BallRadiusMeters * PixelsPerMeter));
              let xcur = x;
              let ycur = y;
            let prevBall = anchor1;
            for (let i=0; i < nBalls; ++i) {//springInitAngles
        ...
        function Render(sim) {
            ...
            for (let s of sim.springList) {
                ...
                context.moveTo(ScreenHor(s.ball1.x), ScreenVer(s.ball1.y));
                  ...
                  var ycur = s.ball1.y;
                  for (let i = 1; i <= n; i++) {
                      context.lineTo(ScreenHor(xcur + segx + devx), ScreenVer(ycur + segy + devy));
                      context.lineTo(ScreenHor(xcur + segx * 2), ScreenVer(ycur + segy * 2));
                      context.lineTo(ScreenHor(xcur + segx * 3 - devx), ScreenVer(ycur + segy * 3 - devy));
            ...
            for (let b of sim.ballList) {
                if (0 === 0) {
                    ...
                    context.beginPath();
                    context.arc(ScreenHor(b.x), ScreenVer(b.y), b.radius * PixelsPerMeter, 0, 2*Math.PI, true);
                    context.fill();
                    context.stroke();
                } else {
        ...
        function AnimationFrame() {
              ...
              for (let i = 1; i < sims.length; ++i) {
                  for (let b of sims[i].ballList) {
                     if (b.x < xmax + b.radius && b.x > xmin - b.radius && b.y < ymax + b.radius && b.y > ymin - b.radius) {
                          still = true;
                          break;
                    }
                }
            ...
            context.fillText("" + smartrnd(dif1.val, 0, 3, 100) + "%", ScreenHor(xmin + xrng * 0.235), ysync); 
            context.fillText("" + smartrnd(dif2.val, 0, 3, 100) + "%", ScreenHor(xmin + xrng * 0.735), ysync); 
            }
